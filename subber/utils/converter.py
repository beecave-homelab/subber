...
def convert_to_mp3(video_file: Path, output_dir: Path) -> bool:
    ...
    try:
        ...
        if output_path.exists():
            logger.debug("Skipping %s (already exists)", video_file.name)
            return False

        logger.debug("Starting conversion of %s", video_file.name)
        ...
        cmd = [
            ...
            str(video_file),
            "-q:a",
            AUDIO_CONVERSION["FFMPEG_SETTINGS"]["quality"],
            "-map",
            AUDIO_CONVERSION["FFMPEG_SETTINGS"]["map"],
        ...
        try:
            ...
            if return_code == 0:
                logger.debug("Successfully converted %s", video_file.name)
                return True
            else:
                logger.error("FFmpeg process failed with return code %d", return_code)
                # Clean up incomplete file on failure
                if output_path.exists():
                    output_path.unlink()
                    logger.debug("Cleaned up incomplete file: %s", output_path)
                return False
        ...
        logger.error("Failed to convert %s: %s", video_file.name, str(e))
...
def batch_convert_to_mp3(
    video_files: List[Path], output_dir: Optional[Path] = None
) -> int:
    """
    ...
    logger.debug("Created/verified output directory", extra=_format_path(output_dir))
    ...
    logger.debug(
        "Found files available for conversion",
        extra={"file_count": len(file_choices)}
    )

    try:
        selected = questionary.checkbox(
            "Select which video files to convert to MP3:", choices=choices
        ).ask()
        ...
        if "ALL" in selected:
            ...
            logger.debug(
                "Selected all files for conversion",
                extra={"file_count": len(files_to_convert)}
            )
        else:
            files_to_convert = selected
            logger.debug(
                "Selected specific files for conversion",
                extra={"file_count": len(files_to_convert)}
            )
