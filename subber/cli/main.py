...
@click.option(
    "-n", "--no-table", is_flag=True, help="Output results without table formatting"
)
@click.option(
    "-p", "--path", is_flag=True, help="Show the full path of the files in the output"
)
@click.option(
    "-m", "--move-unmatched", help="Folder to move unmatched video files into"
)
@click.option(
    "-r",
    "--rename",
    is_flag=True,
    help="Interactively rename close-matched subtitle files to match video names",
)
@click.option(
    "-c",
    "--convert",
    is_flag=False,
    flag_value=AUDIO_CONVERSION["DEFAULT_OUTPUT_DIR"],
    default=None,
    help=f"Convert selected video files to MP3 format. Optionally specify output "
    f"subdirectory (default: '{AUDIO_CONVERSION['DEFAULT_OUTPUT_DIR']}')",
)
@click.option(
    "-v", "--verbose", is_flag=True, help="Enable verbose output with debug information"
)
@click.option(
    "--log-file",
    type=click.Path(),
    help="Specify a custom log file path (default: timestamped file in subber_logs/)",
)
def main(
    directory,
    output_file,
    ...
    # 2. Match them
    exact_matches, close_matches, unmatched_videos, unmatched_subtitles = find_matches(
    ...
    if convert is not None and unmatched_videos:
        if not check_ffmpeg_installed():
            console.print(
                MESSAGES["FFMPEG_NOT_INSTALLED"], style=CONSOLE_STYLES["error"]
            )
            return
        ...
        converted = batch_convert_to_mp3(
            [Path(v) for v in unmatched_videos], convert_output_dir
        )
        if converted > 0:
            console.print(
                f"\nSuccessfully converted {converted} files to MP3.",
                style=CONSOLE_STYLES["success"],
            )
            console.print(
                f"MP3 files saved in: {convert_output_dir}",
                style=CONSOLE_STYLES["success"],
            )
